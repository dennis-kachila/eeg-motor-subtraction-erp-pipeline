...
//		void *VarChanInfoPos = hdr->AS.Header + datapos + 30;  // unused
		...
		for (m = 0; m < NumberOfDataSections; m++) {
			...
			for (k = 0; k < d; k++) {
				...
				char   *desc = (char*)(hdr->AS.Header+pos+1);
				uint16_t typ = leu16p(hdr->AS.Header+pos+22);
				int  unitlen = (uint8_t)hdr->AS.Header[pos+24];
				char   *unit = (char*)(hdr->AS.Header+pos+25);
				uint16_t off = leu16p(hdr->AS.Header+pos+34);
				...
				else if ((k==3) && (typ==6) && !strcmp(desc,"Start") && (unitlen>0) && !strncmp(unit,"s",unitlen))
					tmp_event_pos = f;
...
if (VERBOSE_LEVEL>7) 		{
			...
			for (k = 0; k < hdr->NS; k++) {
				...
				double Cal   = lef32p(pos+8);
				double Off   = lef32p(pos+12);
				...
				else if (xspr==0 && hc->OnOff)
					if (VERBOSE_LEVEL>7) fprintf(stdout,"Warning : #%i of section %i contains %i samples (%d)\n", (int)k, (int)m, (int)xspr,  hc->OnOff);
				...
				if (m > 0) {
					...
					case 18:  	// quadruple
					default: ;
						fprintf(stderr, "ERROR: datatype %d in #%i of section %i not supported\n", hc->GDFTYP, (int)k, (int)m);
					}
				}
				else {
					hc->Cal = Cal;
		...
		for (m = 0; m < NumberOfDataSections; m++) {
			...
			for (k = 0; k < hdr->NS; k++) {
				...
				if (xspr0 == 0)
					...
					if (VERBOSE_LEVEL>7) fprintf(stdout,"%s line %d: Warning : #%i of section %i contains %i samples (OnOff=%d)\n", __FILE__, __LINE__, (int)k, (int)m, (int)xspr, hc->OnOff);
	...
	if (VERBOSE_LEVEL>7) fprintf(stdout,"%s (line %i) #%i %i,%i %i %i %i %i: %i @%p %i\n",
					...
					size_t k2;
					if (gdftyp == hc->GDFTYP && Cal==hc->Cal && Off==hc->Off) {
